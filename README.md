...
- **三语完整剧情语料**：`corpus/zh/`（中文 156 文件）+ `corpus/jp/`（日语 570 文件）+ `corpus/en/`（英语 547 文件），支持切换中/日/英对话语言，含三语索引与使用协议
...
2. **语料**：`corpus/` 为剧情中/日/英台词语料（源自 pjsk.moe 三服数据层），随仓库分发为用户自主决定，仅供学习研究，**请勿商用**；版权方要求时应予下架。
