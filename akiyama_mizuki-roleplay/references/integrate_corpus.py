# -*- coding: utf-8 -*-
"""三语语料索引构建器（v4 · 适配英文文件名 + zh/ja/en 后缀）

用法：
  python integrate_corpus.py index-only          # 从 corpus/{zh,ja,en} 重建索引（常用）
  python integrate_corpus.py --sync-lang zh      # 从源目录同步指定语言后重建
  python integrate_corpus.py --sync-lang all     # 同步全部三种语言

文件名约定（与 corpus/ 实际结构一致）：
  main-21-nightcord-at-25.zh.md
  event-002-caged-marionette.ja-1.ja.md   # ja 多文件带 .ja-N 分段
  intro-1.zh.md
  area-as_1_058.zh.md
语言后缀固定为 .zh.md / .ja.md / .en.md
"""
import os, re, json, shutil, sys

SKILL = r"C:\Users\1\.workbuddy\skills\akiyama_mizuki-roleplay"
...
MIZUKI_NAME = {"zh": "瑞希", "ja": "瑞希", "en": "Mizuki"}
SUB_DIRS = ("main", "event", "self-intro", "area-talk")
SPEAKER_RE = re.compile(r"^【([^】]+)】")

# 语言后缀 -> 语言键
SUFFIX_LANG = {".zh.md": "zh", ".ja.md": "ja", ".en.md": "en"}


def split_lang(fn):
    """从文件名拆出 (base, lang)。base 已去除语言后缀与 ja 分段标记。"""
    for suf, lang in SUFFIX_LANG.items():
        if fn.endswith(suf):
            base = fn[: -len(suf)]
            base = re.sub(r"\.ja-\d+$", "", base)  # 去除 event 的 .ja-1 分段
            return base, lang
    return None, None
...
def collect_lang(lang):
    ...
    if os.path.isdir(ar_dir):
        for fn in sorted(os.listdir(ar_dir)):
            ...
            t, mz, sp = scan_file(p, mizuki)
            entries.append({
                "lang": lang, "type": "area", "id": base, "title": base,
                "path": f"corpus/{lang}/area-talk/{fn}",
                "lines": t, "mizuki_lines": mz, "cast": sorted(sp.keys()),
            })
    return entries
...
def main():
    ...
    if "--sync-lang" in args:
        ...
        langs = ("zh", "ja", "en") if lang == "all" else (lang,)
        for l in langs:
            sync_lang(l)
    all_entries = []
...
def write_md(entries):
    # 按 (type, id, lang) 聚合：活动按 id 合并（ja/en 每话一文件），主线/自我介绍/区域对话 1:1
    by_id = {}
    for e in entries:
    ...
    def get(lang, typ, tid):
        ...
        o = dict(d)
        if o.get("_count", 1) > 1:
            o["title"] = f"{o['title']} ほか{o['_count']}話" if lang == "ja" else (f"{o['title']} +{o['_count']-1}eps" if lang == "en" else o["title"])
        return o
    ...
    md.append("")
    md.append("> 来源：pjsk.moe 数据层（zh/ja/en 三服）。格式：`【角色名】台词`；括号内为内心独白；无标记行为旁白。")
    md.append("> ⚠️ 版权：角色与剧情归 SEGA / Colorful Palette，语料仅供学习研究，请勿商用；版权方要求时下架。")
    md.append("")
    md.append("## 使用协议（模型必读）")
    md.append("")
    md.append("1. **语言切换**：默认中文。用户要求用日语/英语交流时 → 改读 `corpus/ja/` 或 `corpus/en/` 对应文件校准口吻与称呼，并**用该语言扮演**；切回中文同理。语料路径 = `corpus/{zh|ja|en}/…`。")
    md.append("2. **先查索引，再读文件**：按 (type, id) 定位三个语言的文件路径，再 Read 对应单文件（500-1,600 行，禁止全量加载）。")
    md.append("3. **说话人变体**：`【瑞希】`（zh/ja）/`【Mizuki】`（en）是本人；`【小学生瑞希】`/`【Kid Mizuki】` 为幼年期；`【优希】`=姐姐、`【瑞希的母亲】`/`【Mizuki's Mother】` 等家人**不是**瑞希；`【A&B&瑞希】` 为多人合说。")
    ...
    md.append("")
    md.append("> 路径：`corpus/{zh,ja,en}/area-talk/{sid}.md`；完整字段（含 cast）见 `corpus_index.json`。")
    md.append("")
